function [X, y] = preprocess(path_to_dataset, histogram, count_bins)
  ...
  for i = 1 : Cats
    if histogram == "RGB"
      X(i, :) = rgbHistogram(PATH1(i, :)(~isspace(PATH1(i, :))), count_bins);
      %pe linia i a lui X, vectorul de caracteristci al imaginii i
     endif
    if histogram == "HSV"
      X(i, :) = hsvHistogram(PATH1(i, :)(~isspace(PATH1(i, :))), count_bins);
    endif   
  endfor
  ...
  for i = (Cats + 1) : N
    y(i) = -1; 
    if histogram == "RGB"
      X(i, :) = rgbHistogram(PATH2((i - Cats), :)(~isspace(PATH2((i - Cats), :))), count_bins);
      %pe linia i a lui X, vectorul de caracteristci al imaginii i
     endif
    if histogram == "HSV"
      X(i, :) = hsvHistogram(PATH2((i - Cats), :)(~isspace(PATH2((i - Cats), :))), count_bins);
    endif   
  endfor
